...
phylo.spindles<-function(tree0, occ, ages=NULL, xlimits=c(300,0), col=add.alpha("black"), fill=col,lwd=1,dscale=0.002,res=1, cex.txt=1,col.txt=add.alpha(col,1), axis=T, labels=T, txt.y=-0.4,txt.x=150, add=FALSE,tbmar=0.2,weights=1){
...
#limits for spindles based on ages
...
#plot spindles
viol(seq(tmp1$x.lim[1],tmp1$x.lim[2],res),pos=i, stat=divdistr_, table=convert.sptab(eval(parse(text=paste0("occ$sptab_",tree0$tip.label[i]))), tree0), dscale=dscale, col=col, fill=fill, lwd=1,cutoff=cutoff,w=w)
