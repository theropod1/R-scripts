...
ini_<-function(x, options, match=NULL, ignore.case=TRUE, substitute=FALSE,...){
TRUE->res2 #matches match (assumed true if match==NULL)

if(!is.null(match)){

grepl(x, substr(match,1,nchar(x)),ignore.case=ignore.case,...)->res2
...
if(res2==FALSE){return(FALSE)#if match is given and x is not in match, return FALSE
    ...
    if(match%in%options){
    grep(x,substr(options,1,nchar(x)),ignore.case=ignore.case,...)->res1
    }else{
    grep(x,substr(c(match,options),1,nchar(x)),ignore.case=ignore.case,...)->res1
    }
...
logical()->result
for(i in 1:length(x)){
result[i]<-ini_(x[i], options, match[i], ignore.case=ignore.case, substitute=substitute,...)}
return(result)
}else{
return(ini_(x, options, match, ignore.case=ignore.case, substitute=substitute,...))
}
