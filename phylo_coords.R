...
phylo_coords <- function(tree,ages=NULL,add.terminal=TRUE, plot=FALSE, type="cladogram",add=TRUE,yscale=c(1,1),root.edge=FALSE, horiz=TRUE, ...) {
...
if(type!="cladogram"){
if(horiz){
arrows(x0=x_[tree$edge[,1]], y0=y_[tree$edge[,1]], x1=x_[tree$edge[,1]], y1=y_[tree$edge[,2]],...)
arrows(x0=x_[tree$edge[,1]], y0=y_[tree$edge[,2]], x1=x_[tree$edge[,2]], y1=y_[tree$edge[,2]],...)}else{

arrows(x0=x_[tree$edge[,1]], y0=y_[tree$edge[,1]], x1=x_[tree$edge[,2]], y1=y_[tree$edge[,1]],...)
arrows(x0=x_[tree$edge[,2]], y0=y_[tree$edge[,1]], x1=x_[tree$edge[,2]], y1=y_[tree$edge[,2]],...)
}

}else if(type=="cladogram") arrows(x_[tree$edge[,1]], y_[tree$edge[,1]], x_[tree$edge[,2]], y_[tree$edge[,2]],...)

if(root.edge!=FALSE & is.numeric(root.edge)) if(horiz) arrows(x_[root]+root.edge, y_[root], x_[root], y_[root],...) else arrows(x_[root], y_[root]+root.edge, x_[root], y_[root],...)
...
arrows(x[tree$edge[,1]], y[tree$edge[,2]], x[tree$edge[,2]], y[tree$edge[,2]],...)}else{

arrows(x[tree$edge[,1]], y[tree$edge[,1]], x[tree$edge[,2]], y[tree$edge[,1]], ...)
arrows(x[tree$edge[,2]], y[tree$edge[,1]],x[tree$edge[,2]], y[tree$edge[,2]], ...)}

}else if(type=="cladogram") arrows(x0=x[tree$edge[,1]], y0=y[tree$edge[,1]], x1=x[tree$edge[,2]], y1=y[tree$edge[,2]],...)

if(root.edge!=FALSE & is.numeric(root.edge)) if(horiz) arrows(x[root]-root.edge, y[root], x[root], y[root],...) else arrows(x[root], y[root]-root.edge, x[root], y[root],...)
