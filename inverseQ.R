...
#' @param errorterms_x_i Optional: vector of error terms (standard deviations/randomly distributed standard errors) from which to resample a random error term to add to each x_i value in the bootstrap
...
inverseQ<-function(x,x_i,w=1, w_i=1, bootstrap=FALSE, reps=1000, return_reps=FALSE,errorterms_x=0, errorterms_x_i=0){
list()->out
...
if(bootstrap){
if(length(errorterms_x)<length(x)) rep(errorterms_x,length(x))[1:length(x)]->errorterms_x
if(length(errorterms_x_i)<length(x_i)) rep(errorterms_x_i,length(x_i))[1:length(x_i)]->errorterms_x_i
...
x_i_<-x_i[x_i_]+rnorm(n=length(indices),mean=0,sd=errorterms_x_i[x_i_]) #XXX
sum(w_[x_<=x_i_])/sum(w_)->q
return(q)
}
replicate(reps,iq(x,w)) -> out$bootstrap_res #perform steps in iq reps times and save result

quantile(out$bootstrap_res,c(0.005,0.01,0.025,0.05,0.1,0.9,0.95,0.975,0.99,0.995),na.rm=TRUE)->out$bootstrap_CIs #compute confidence interval quantiles

median(out$bootstrap_res,na.rm=T)->out$bootstrap_median
mean(out$bootstrap_res,na.rm=T)->out$bootstrap_mean
